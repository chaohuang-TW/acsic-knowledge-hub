...
import { getEconomies } from '../institutions/directoryUtils';
...
function economyLabel(countryCode: string) {
  const label = economyLabels.get(countryCode);
  if (!label) throw new Error(`Unknown governed economy: ${countryCode}`);
  return label;
}

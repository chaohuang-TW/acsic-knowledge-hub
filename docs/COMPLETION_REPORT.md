...
- Level 1 完成 21／21；依單一版本化共通與機構角色別標準計算，嚴格 Level 2 為 4 筆完整、16 筆部分完成、1 筆證據不足。Level 3 值與來源由單一 production 資料契約治理，涵蓋 JFC、ACGF 與 TSMEG。
...
- `pnpm check`: passed (generated-document consistency, format, lint, typecheck, 87 unit/data-governance tests, build and secret scan).
- `pnpm test:e2e`: passed, 36/36 across desktop and mobile Chromium, including all 21 detail views, both languages, filters, comparison, reference set, indicator framework and downloads.
...
- Strict Level 2: 4 complete, 16 partial and 1 insufficient. Verified Level 3 pilot records: 18.
- 76 source-reference relationships resolve to 59 unique official sources and 224 field-evidence objects.
